...
def _cdx(*rows):
    ...
    body = [
        [ts, "https://example.cn/story", status, digest, "text/html", "100"]
...
def test_status_class_maps_codes():
    ...
    )  # revisit/dedup row ⇒ never drives a transition
...
def test_parse_cdx_json_sorts_and_survives_garbage():
    ...
    assert [c.timestamp for c in caps] == [
        "20220101000000",
        "20220301000000",
    ]  # sorted by time
...
def test_snapshot_urls_are_citable_evidence():
    ...
    assert (
        cap.snapshot_url()
        == "https://web.archive.org/web/20220101120000/https://example.cn/story"
    )
...
def test_reconstruct_detects_deletion_with_bracket():
    ...
    assert (
        "20220301000000" in rec.primary.detail
        and "20220401000000" in rec.primary.detail
    )
    assert rec.note == ""
        ...
        parse_cdx_json(
            _cdx(
                ...
                (
                    ...
                    "200",
                    "BBBB",
                ),  # content digest changed ⇒ silent redaction
            )
        ),
        term="notice",
    )
...
def test_vantage_refuses_when_killswitched(tmp_path):
    ...
    ks.engage("test")
    called = []
    vp = WaybackVantagePoint(
        fetch_cdx=lambda u: called.append(u) or "[]", kill_switch=ks
    )
    try:
...
def test_vantage_preserves_requested_url_for_a_valid_empty_response(payload):
    ...
    rec = WaybackVantagePoint(fetch_cdx=lambda _url: json.dumps(payload)).observe(
        url, term="watched"
    )
...
def test_vantage_applies_time_window_clientside():
    payload = _cdx(
        ("20200101000000", "200", "AAAA"),
        ("20220101000000", "200", "AAAA"),
        ("20220401000000", "404", "-"),
    )
    vp = WaybackVantagePoint(
        fetch_cdx=lambda u: json.dumps(payload), from_ts="20211231000000"
    )
...
def test_reconstruction_flows_into_ddti_index():
    ...
    assert obs["source"].startswith(
        "undertext:wayback:"
    )  # the shared adapter, wayback surface
...
def test_runner_observations_merge_into_live_pull(tmp_path):
    ...
    pytest.importorskip(
        "httpx",
        reason="scripts.ddti_live_pull needs the collector "
        ...
        "ddti_observations": [
            {
                ...
                "title": "[undertext:deletion] 某地 挤兑",
                "url": "",
                "source": "undertext:wayback:x",
            },
            {
                "terms": [],
                "detected_at": "2026-07-01T00:00:00+00:00",
...
def test_cdx_query_url_uses_digest_collapse():
    url = cdx_query_url(
        "https://example.cn/story", from_ts="20220101", to_ts="20221231"
    )
