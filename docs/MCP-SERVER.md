...
| `get_signal(name, max_rows=25)` | One signal's full latest reading, exactly as served on the site, with its `generated_at` and upstream sources. This is the door to model evaluation (`eval-registry`, `refusal-drift`) and the named-series economic backtest (`china-econ-forecast`). |
