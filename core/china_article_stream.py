    ...
    for event in wire["events"]:
        ...
        if analysis is None:
            raise ChinaArticleStreamError(
                f"missing event analysis for {event['event_id']}"
            )
        ...
        entries.append(
            {
                ...
                "language": _language(item["title"] + " " + item["excerpt"], item["source_id"]),
                "desk": item["desk"],
...
def validate_china_article_stream(document: Mapping[str, Any]) -> None:
    """Validate identity, ordering, linkage, and the public privacy boundary."""
    ...
    if document.get("coverage", {}).get("china_entries") != len(entries):
    ...
    for index, entry in enumerate(entries):
        ...
        if not _EVENT_ID_RE.fullmatch(str(dossier.get("event_id", ""))):
