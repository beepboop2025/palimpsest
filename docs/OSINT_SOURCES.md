...
| **Attributed peer-context warehouse** (`core/peer_context.py`, fleet `peer-context`) | Offline join of GreatFire cache + OONI disk + CDT RSS + Weiboscope citation | Per-host / per-event `peer_context` rows in canned voice (“GreatFire’s 90-day verdict… as of…”, “OONI has N China measurements…”, “CDT published a related title… Palimpsest did not write that piece.”) | Never claims a peer row as Palimpsest capture. Never motive. Latest files stay on the box; do not commit hollow placeholders |
