...
HONEST LIMITS, stated because they bound what the numbers mean:
  ...
  - A layer with one signal in it (MODEL, today) is not corroborated internally.
    That is disclosed in the output rather than being papered over by the mean.
...
def build_reading(readings_dir: str | Path, *, alpha: float = FDR_ALPHA) -> dict:
    ...
    return {
        ...
        "method": (
            "per-signal two-sided conformal Shiryaev-Roberts e-detectors; e-BH "
        ...
        "caveats": [
            ...
            "a layer holding one signal cannot corroborate itself — corroborated=false "
            "marks those, and today that is the MODEL layer",
            "signals still warming up contribute no e-value; they are neither "
            "evidence of calm nor of change",
        ],
    }
