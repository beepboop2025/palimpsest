...
def _renderer_pages() -> list[Path]:
    return [
        path
        for path in _html_pages()
        if "<!-- GENERATED BY scripts/build_china_site.py -->"
...
def test_nested_sitemap_contains_every_html_canonical_once():
    ...
    canonicals = [_parser(path).canonicals[0] for path in _renderer_pages()]
    counts = _index()["counts"]
    expected_pages = (
        4 + counts["sources"] + counts["release_monitors"] + counts["domains"]
    )
