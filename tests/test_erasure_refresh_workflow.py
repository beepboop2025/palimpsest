...
def test_a_race_rebuilds_shared_seals_without_requerying_paid_models():
    ...
    retained_ingest = text.index(
        "python -m scripts.ingest_refusal_drift", legacy_ingest
    )
...
def test_each_retry_discards_stale_derived_bytes_before_rebuilding():
    text = WORKFLOW.read_text(encoding="utf-8")
    race = text.index("Reattest, reverify, and retry after a push race")
    preserve_start = text.index(
        "# Carry forward only this run's measured/model artifacts", race
    )
    ...
    verify_seal = retry.index(
        "python3 scripts/seal_readings.py --check", verify_transcripts
    )
...
def test_every_advanced_base_restores_the_public_registry_then_replays_measurement():
    ...
    conflict_carry = conflict[
        conflict.index("for p in ") : conflict.index("            done")
    ]
    assert "readings/eval-registry.jsonl" not in conflict_carry
    assert "readings/eval-registry-latest.json" not in conflict_carry

    rebuild = sync[
        sync.index("Rebuild the aggregate and shared seal after an input change") :
    ]
...
def test_paid_measurement_is_retained_briefly_after_semantic_verification():
    ...
    assert (
        "actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a" in retained
    )
...
def test_eval_article_scenarios_gate_reconciliation_and_every_race_retry():
    ...
    reconcile_check = text.index(
        "python -m scripts.build_eval_findings --check", reconcile
    )
