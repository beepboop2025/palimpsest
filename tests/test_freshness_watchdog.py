...
def test_osint_deadlines_catch_configured_optional_but_ignore_disabled_or_absent() -> None:
    signals = (
        {
            "id": "bleedthrough",
            "status": "live",
...
def test_deployment_installs_and_verifies_both_watchdog_units_after_api_probe() -> None:
    ...
    assert probe < install_service < verify < enable
    verification = guide[verify:enable]
    ...
    assert "ExecMainStatus" in guide[enable:]
