#!/usr/bin/env python3
"""Peer-context review ranker.

Writes ``peer-context-rank-latest.json`` (``n_peer_series``). Never writes the
warehouse filename. Features come from cached peer-warehouse files when present.
No catalog crawl, no Weiboscope 2012 dump, no generative brief.
"""
...
LATEST = READINGS / "peer-context-rank-latest.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if KillSwitch().is_halted():
        print("peer-context-rank: global kill switch is engaged", file=sys.stderr)
        return 2
    ...
    warehouse = (Path(args.root) if args.root else ROOT) / "readings" / "peer-context-latest.json"
    if warehouse.is_file():
        try:
            loaded = json.loads(warehouse.read_text(encoding="utf-8"))
        except (OSError, ValueError):
            loaded = None
        if isinstance(loaded, dict) and loaded.get("job") == JOB:
            print("peer-context-rank: refused to leave ranker bytes in the warehouse file", file=sys.stderr)
            return 2
    summary = {
        "schema": SCHEMA,
        "job": JOB,
