...
| Telegram China-Myanmar Borderlands Observatory | `scripts/telegram_public_channels_pull.py` | fleet `telegram-public-channels` | Registry schema v3 retains 50 reviewed locators but collection requires `project-owned`, `explicit-consent`, or `licensed` authority. The current collectable set is only the three project-owned Dragon Den channels; every external newsroom, state-media, conflict-party, aggregator, advocacy, candidate, and quarantine entry is discovery-only and is rejected before fetch. Public availability is not permission. No groups, participants, engagement, DMs, or media binaries. See `docs/TELEGRAM-BORDERLANDS-OBSERVATORY.md`. |
